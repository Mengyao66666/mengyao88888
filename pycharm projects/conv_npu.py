            ...
            for out_w in nl.affine_range(out_pool_width):
                ...
                for fh in nl.affine_range(filter_height):
                    for fw in nl.affine_range(filter_width):
                        ...
                        weight_tile_mm[...] = W_tile[i_oc, i_ic, fh, fw]

                        input_tile_mm = nl.ndarray(
                            (in_channels, 1),
                            dtype=X.dtype,
                            buffer=nl.sbuf
                        )
                        i_ic_col = nl.arange(in_channels)[:, None]
                        input_tile_mm[...] = x_tile[i_ic_col, out_h + fh, out_w + fw]

                        # 然后用 mgrid 索引做 matmul
                        result = nisa.nc_matmul(
                            weight_tile_mm[weight_mgrid.p, weight_mgrid.x],
                            input_tile_mm[input_mgrid.p, input_mgrid.x]
                        )
